...
void spr_geo_reco()
{
  ...
  Double_t spr_ti_standalone[25];
  Double_t spr_ti_err_standalone[25];
  ...
  for(int j=0; j < 25; j++)
    {      
      ...
      spr_ti_standalone[j] = spr[2];
      spr_ti_err_standalone[j] = 0.;
  
    }

  TGraphErrors* gr1 = new TGraphErrors(25, theta, spr_ti_standalone, theta_err, spr_ti_err_standalone);

  gr1->SetTitle("Single Photon Resolution at 6 GeV/c - time imaging reco.");
